...
train_dataset = dataset[0:int(0.6*data_nm),:].astype(np.float32).reshape([-1,1,data_dim,1])
train_labels = labels[0:int(0.6*data_nm),:].astype(np.float32)
valid_dataset = dataset[int(0.6*data_nm):int(0.8*data_nm),:].astype(np.float32).reshape([-1,1,data_dim,1])
valid_labels = labels[int(0.6*data_nm):int(0.8*data_nm),:].astype(np.float32)
test_dataset =  dataset[int(0.8*data_nm):,:].astype(np.float32).reshape([-1,1,data_dim,1])
...
with graph.as_default():
	...
	weights1 = tf.Variable(tf.truncated_normal([315,hidden_nm1], stddev=1.0))
