...
with graph.as_default():
	...
	def reconstruction(data):
		...
		return representation, pred_data

	representation, pred_data = reconstruction(tf_train_dataset)
	l2 = tf.nn.l2_loss(representation)
	l1 = tf.reduce_mean(tf.pow(tf_train_dataset - pred_data, 2))
	cost = l1 + 0.1*l2
	optimizer = tf.train.AdamOptimizer().minimize(cost)

	_,valid_prediction = reconstruction(tf_valid_dataset)
	_,test_prediction = reconstruction(tf_test_dataset)
...
with tf.Session(graph=graph) as session:
	...
	print("Model saved in file: %s" % save_path)
 	...
 	for i_test in np.random.randint(test_dataset.shape[0],size=10).tolist():
		...
		_,prd_test = reconstruction(test_dataset[i_test,:].reshape(-1,25))
		prd_test = prd_test.eval()[0,:]
