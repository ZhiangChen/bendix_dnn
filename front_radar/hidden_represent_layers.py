...
with graph.as_default():
	...
	weights1_f = tf.Variable(tf.truncated_normal([f,hidden_nm1_f], stddev=1.0))
	biases1_f = tf.Variable(tf.zeros([hidden_nm1_f]))

	weights1_r = tf.Variable(tf.truncated_normal([r,hidden_nm1_r], stddev=1.0))
	biases1_r = tf.Variable(tf.zeros([hidden_nm1_r]))

	weights2_f = tf.Variable(tf.truncated_normal([hidden_nm1_f,hidden_nm2], stddev=1.0))
	biases2_f = tf.Variable(tf.zeros([hidden_nm2]))

	weights2_r = tf.Variable(tf.truncated_normal([hidden_nm1_r,hidden_nm2], stddev=1.0))
	biases2_r = tf.Variable(tf.zeros([hidden_nm2]))
	...
	def model(data):
		data_f = data[:,0:f]
		data_r = data[:,f:]
		hidden_in_f = tf.matmul(data_f, weights1_f) + biases1_f
		hidden_in_r = tf.matmul(data_r, weights1_r) + biases1_r
		hidden_out_f = tf.nn.relu(hidden_in_f)
		hidden_out_r = tf.nn.relu(hidden_in_r)
		hidden_in_f = tf.matmul(hidden_out_f,weights2_f) + biases2_f
		hidden_in_r = tf.matmul(hidden_out_r,weights2_r) + biases2_r
		hidden_in = hidden_in_f + hidden_in_r
